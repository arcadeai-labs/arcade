...
- **Fix a connection** (wrong account, expired sign-in, missing permissions,
  "reconnect"): use `Arcade_Apps` with `action: "status"` to check, then
  `"switch_account"` or `"reauthorize"` (with the `provider_id` from status)
  to get a fresh sign-in link. Present the link, stop, and wait for the
