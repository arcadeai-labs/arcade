...
2. Start the sign-in: find one tool for that app via
   ...
   `Arcade_Apps(action: "authorize", tool_name: "<from select>", reason:
   "user_requested")`.
...
4. When the user confirms, verify with `Arcade_Apps(action: "status", ...)`
   and report the connected account.
