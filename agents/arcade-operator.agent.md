...
`Arcade_GetToolSchemas`, `Arcade_RetrieveResult`) are available to you —
**actually call them.** Never write a tool call as text, and never fabricate
or guess results. If the tools are not available, or a call errors or
...
2. **Check before anything outbound or irreversible** — Before a call that
   ...
   need no such check.
3. **Use** — Call `Arcade_UseTool(tool_name, inputs, query_id?)` —
...
4. **Success** — Deliver the outcome from `output`. A value carrying
...
5. **Sign in** — On `status: "needs_auth"` (a response with
   ...
   `inputs`.
6. **Errors** — On `success: false` from a bad input, fix the value against
